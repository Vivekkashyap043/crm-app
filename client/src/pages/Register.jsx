        ...
        <div className="row justify-content-center align-items-center">
          ...
          <div className="col-lg-5">
            <div className="card p-4 shadow-lg border-0" style={{ borderRadius: 16, minWidth: 340, maxWidth: 400, margin: '0 auto' }}>
              <h2 className="mb-2 text-center fw-bold" style={{ color: '#1e293b' }}>Create your account</h2>
              <div className="mb-3 text-center text-muted" style={{ fontSize: 15 }}>Sign up to get started with CRM Pro</div>
              <form onSubmit={handleSubmit}>
                <div className="mb-3">
                  <input type="text" className="form-control form-control-lg" placeholder="Name" value={name} onChange={e => setName(e.target.value)} required />
                </div>
                <div className="mb-3">
                ...
                <button type="submit" className="btn w-100 fw-bold" style={{ background: 'linear-gradient(90deg, #2563eb 0%, #1e40af 100%)', color: '#fff', fontSize: 18 }}>Sign Up</button>
                {error && <div className="alert alert-danger mt-3">{error}</div>}
              </form>
              <div className="mt-3 text-center">
                <span className="text-muted">Already have an account? </span>
                <button className="btn btn-link p-0 fw-bold" style={{ color: '#2563eb' }} onClick={() => navigate('/login')}>Sign In</button>
              </div>
            </div>
          </div>
        </div>
